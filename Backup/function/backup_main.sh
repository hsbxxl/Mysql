...
scp -o "StrictHostKeyChecking no" $server_backup_script_temp/create_path_script.sh ${client_user}@"${host_name}":$client_backupset_temp
ssh ${client_user}@${host_name} 'sh $client_backup_log_temp/create_path_script.sh >/dev/null 2>&1 &' 
...
backup_stat=`ssh ${client_user}@${host_name} 'cat $server_backup_log/status.log'`
   ...
   then
	  ...
	  mv $client_backup_log_temp/${host_name}_$(date +%F)*_output.log $client_backup_log/${host_name}_$(date +%F)*_output.log
   else
